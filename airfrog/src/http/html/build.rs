...
impl HtmlBuilder {
    ...
    pub fn h1(mut self, text: &str) -> Self {
        self.content.push_str(&format!("<h1>{}</h1>", escape_html(text)));
        self
    }
    
    pub fn h2(mut self, text: &str) -> Self {
        self.content.push_str(&format!("<h2>{}</h2>", escape_html(text)));
        self
    }
    ...
    where F: FnOnce(TableBuilder<Open>) -> TableBuilder<Open>
    {
    ...
    pub fn script_src(mut self, src: &str) -> Self {
        self.content.push_str(&format!("<script src=\"{}\"></script>", escape_html(src)));
        self
    }
    
    pub fn script(mut self, code: &str) -> Self {
        // Script content should not be HTML escaped as it's JavaScript
        self.content.push_str(&format!("<script>{}</script>", code));
        self
    }
    
    pub fn link(mut self, href: &str, text: &str) -> Self {
        self.content.push_str(&format!("<a href=\"{}\">{}</a>", escape_html(href), escape_html(text)));
        self
    }
    
    pub fn button(mut self, text: &str, onclick: &str) -> Self {
        self.content.push_str(&format!("<button onclick=\"{}\">{}</button>", escape_html(onclick), escape_html(text)));
        self
    }
    
    pub fn button_plain(mut self, text: &str) -> Self {
        self.content.push_str(&format!("<button>{}</button>", escape_html(text)));
        self
    }
...
impl ElementBuilder {
    ...
    where F: FnOnce(HtmlBuilder) -> HtmlBuilder 
    {
        // Open tag with attributes
        let attrs = self.attributes.iter()
            .map(|(k, v)| format!(" {}=\"{}\"", k, escape_html(v)))
            .collect::<String>();
        self.builder.content.push_str(&format!("<{}{}>", self.tag, attrs));
    ...
    pub fn text(mut self, text: &str) -> HtmlBuilder {
        let attrs = self.attributes.iter()
            .map(|(k, v)| format!(" {}=\"{}\"", k, escape_html(v)))
            .collect::<String>();
        self.builder.content.push_str(&format!("<{}{}>{}</{}>", self.tag, attrs, escape_html(text), self.tag));
        self.builder
    }
}
...
impl TableBuilder<Open> {
    ...
    where F: FnOnce(RowBuilder<Open>) -> RowBuilder<Open>
    {
...
impl RowBuilder<Open> {
    ...
    pub fn cell(mut self, content: &str) -> Self {
        let width_attr = self.pending_width
            .map(|w| format!(" style=\"width: {};\"", escape_html(&w)))
            .unwrap_or_default();
        
        self.content.push_str(&format!("<td{}>{}</td>", width_attr, escape_html(content)));
    ...
    pub fn label_cell(mut self, content: &str) -> Self {
        ...
        self.content.push_str(&format!("<td class=\"label-col\"{}><strong>{}</strong></td>", width_attr, escape_html(content)));
        self.pending_width = None; // Width only applies to next cell
        self
    }
    ...
    pub fn link_cell(mut self, href: &str, text: &str) -> Self {
        ...
        self.content.push_str(&format!("<td{}><a href=\"{}\">{}</a></td>", width_attr, escape_html(href), escape_html(text)));
        self.pending_width = None; // Width only applies to next cell
        self
    }
